...
plotLPD(computeWPD(nnet.mod$learner.model, dt, "x5", wp = 2))
plotLPD(computeWPD(nnet.mod$learner.model, dt, "x5", wp = 0)) # wp=0 --> non-weighted partial dependence
...
plotLPD(computeWPD(nnet.mod$learner.model, dt, "x2", wp = 1))
...
PD = computeWPD(nnet.mod$learner.model, dt, "x5", wp = 2)
plotLPD(PD)
x.grid.dist = diff(PD$x.grid)[1]
y.hat.diffs = diff(PD$y.hat) / x.grid.dist

plot(PD$x.grid, PD$y.hat, type = "l")
abline(v = partition(PD$x.grid[-length(PD$x.grid)] + x.grid.dist/2, y.hat.diffs, 7))

# partition derivative
PDeriv = computeWPD(nnet.mod$learner.model, dt, "x5", wp = 2, derivative = TRUE,
  predict.fun = nnet.pred.fun)
plotLPD(PDeriv)

plot(PD$x.grid, PD$y.hat, type = "p", pch = ".")
...
plotLPD(computeLPD(gbm.mod$learner.model, dt, "x5", predict.fun = gbm.pred.fun))
plotLPD(computeWPD(gbm.mod$learner.model, dt, "x5", wp = 1, predict.fun = gbm.pred.fun))
plotLPD(computeWPD(gbm.mod$learner.model, dt, "x5", wp = 0, predict.fun = gbm.pred.fun))
plotPartialDependence(generatePartialDependenceData(gbm.mod, tsk, "x5", n = 100, individual = FALSE))
...
```{r}
computeLPD(gbm.mod$learner.model, dt, "x5", predict.fun = gbm.pred.fun, derivative = TRUE)$y.hat
...
```{r}
# LPD
PD = computeLPD(gbm.mod$learner.model, dt, "x5", predict.fun = gbm.pred.fun)
plotLPD(PD)
x.grid.dist = diff(PD$x.grid)[1]
y.hat.diffs = diff(PD$y.hat) / x.grid.dist
plot(PD$x.grid, PD$y.hat, type = "l")
...
abline(v = partition(PD$data$x5[-length(PD$data$x5)] + x.grid.dist/2, y.hat.diffs, 10))
