...
```{r}
lm.ime = compAME(lm.mod, df, features = c("x1", "x2"), individual = TRUE)
poly12.ime = compAME(poly12.mod, df, features = c("x1", "x2"), individual = TRUE)
table(lm.ime[[2]])
table(round(poly12.ime[[1]], digits = 3))
table(round(poly12.ime[[2]], digits = 3))

poly12.y.hat = predict(poly12.mod, newdata = df)

p1 = ggplot() +
  geom_line(aes(x = df$x1, y = poly12.ime$x1)) +
  xlab("x1") + ylab("AME") + ggtitle("AME")
p2 = ggplot() + 
  geom_line(aes(x = df$x1, y = poly12.y.hat)) +
  xlab("x1") + ylab("y.hat") + ggtitle("Predicted values")
gridExtra::grid.arrange(p1, p2, nrow = 2)

split.points = partition(df$x1, poly12.ime$x1, 2)
split.points = partition(df$x1[-74], poly12.ime$x1[-74], 2) # ohne Ausreißer
split.points = partition(df$x1[-74], poly12.ime$x1[-74], 7) # ohne Ausreißer
split.points = partition(df$x1[-74], poly12.ime$x1[-74], 7, part.method = "cluster") # ohne Ausreißer

ggplot(df, aes(x = x1, y = y)) + geom_point() +
  geom_point(mapping = aes(x = df$x1, y = poly12.ime$x1, col = "AME")) +
...
unlist(compAME(lm.mod, df, names(df)[-1]))
system.time(computeAME(lm.mod, df, names(df)[-1]))
system.time(compAME(lm.mod, df, names(df)[-1]))

df2 = df
df2$logic1 = as.factor(df2$logic1)
tsk = makeRegrTask(data = df2, target = "y")
lrn = makeLearner("regr.lm")
lm.mod.mlr = train(lrn, tsk)
plotPartialDependence(generatePartialDependenceData(lm.mod.mlr, tsk, "ord1", derivative = TRUE))
```

```{r}
# data generating process without logic:
y = .4 + 2.3 * bin1 + 2.2 * x1 + bin2 * x2 + 3 * x2 + 1.7 * x3^2 + x4 +
  .1 * cat1.map[cat1] + .1 * ord1.map[ord1] + eps
  #ifelse(char1 == "A", 0, ifelse(char1 == "E", -1, 1)) + eps
df = data.frame(y, x1, x2, x3, x4, bin1 = factor(bin1), bin2 = factor(bin2), cat1, ord1)
knitr::kable(head(df))
...
lm.mod.mlr = train(lm.lrn, regr.tsk)
lm.mod.mlr$learner.model
unlist(compAME(lm.mod.mlr, regr.tsk, names(df)[-1]))
...
unlist(compAME(rf.mod.mlr, regr.tsk, names(df)[-1]))
