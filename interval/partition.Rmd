...
ggplot() + geom_point(data=df, mapping=aes(x = x, y = y)) +
  ...
  geom_vline(mapping = aes(xintercept = partition(df$x, df$y, 4, method = "cluster") + .03, color = "cluster"))
```
...
gridExtra::grid.arrange(
  ...
  p0 + geom_vline(mapping = aes(xintercept = partition(df$x, df$y, 20, method = "cluster"), color = "cluster"))
)
```
